...
def get_cache(key):
    """Check if the key is in the cache and return its value if it is not expired."""
    for item in cache:
        if item['key'] == key:
...
def set_cache(key, value, expire_time=CACHE_TIME):
    """Set a key-value pair in the cache with the current timestamp."""
    ...
    cache.append({'key': key, 'value': value, 'timestamp': time.time(), 'expire_time': expire_time})
